/**
 * @license
 * (c) 2021-2022 Hassan Kashani
 * hasan{dot}kashani{at}gmail{dot}com
 *
        ...
        function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
                ...
                if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
                    if ((!rtl && columnPosLeft[i] > range.rightPx) || (rtl && columnPosRight[i] > range.leftPx)) {
                        // All columns to the right are outside the range.
                        break;
                    }
                    if (rtl && columnPosRight[i] > range.leftPx) {
        ...
        function cleanUpAndRenderCells(range) {
            ...
            for (var row = range.top, btm = range.bottom; row <= btm; row++) {
                ...
                for (var i = 0, ii = columns.length; i < ii; i++) {
                    ...
                    if (!rtl && columnPosLeft[i] > range.rightPx) {
                        break;
                    }
